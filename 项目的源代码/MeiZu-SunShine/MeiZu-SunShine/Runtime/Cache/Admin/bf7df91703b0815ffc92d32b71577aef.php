...
<!-- /.navbar-static-side -->
  ...
  <div class="panel-body">
    ...
    <ul>
      ...
      <li style="text-indent:<?php echo ($vo['level']*20); ?>px; list-style-type:none;font-size:20px"><?php echo ($vo['tname']); ?></li>
      </div>
      <?php if($vo['pid'] == 0): ?><div style="width:300px;margin-left:500px;margin-top:-20px;">
        <a href="<?php echo U('Goods/add',array('id'=>$vo['id'],'name'=>$vo['tname']));?>">添加商品</a>
        <span>|</span>
        <a href="<?php echo U('Type/edit',array('id'=>$vo['id']));?>">修改</a>
        <span>|</span>
        <!--<a href="<?php echo U('Type/enable');?>?id=<?php echo ($vo['id']); ?>&status=<?php echo ($vo['status']); ?>
                    "><?php echo ($vo['status']==1?'禁用':'启用'); ?></a>-->

        <a href="<?php echo U('Type/enable',array('status'=>$vo['status'],'id'=>$vo['id'],'childs'=>$vo['childs']));?>" onFocus="this.blur()" class="add"><?php echo ($vo['status'] == 1 ? '启用' : '禁用'); ?></a>
        </div>
      <?php else: ?>
        <div style="width:300px;margin-left:550px;margin-top:-20px;">
        <a href="<?php echo U('Type/goods',array('id'=>$vo['id'],'name'=>$vo['tname']));?>">修改</a>
        <a href="<?php echo U('Type/enable',array('status'=>$vo['status'],'id'=>$vo['id'],'childs'=>$vo['childs']));?>" onFocus="this.blur()" class="add"><?php echo ($vo['status'] == 1 ? '启用' : '禁用'); ?></a>
        <a href="<?php echo U('Details/edit',array('id'=>$vo['id'],'name'=>$vo['tname']));?>">规格参数</a>
        <a href="<?php echo U('Goods/edit',array('id'=>$vo['id'],'name'=>$vo['tname']));?>">基本信息</a>
       <a href="<?php echo U('Images/index',array('id'=>$vo['id'],'name'=>$vo['tname']));?>">商品详情</a>
       <a href="<?php echo U('Images/edit',array('id'=>$vo['id'],'name'=>$vo['tname']));?>">商品展示</a>
